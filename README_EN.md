...
| `api.timeout` | Maximum timeout for a single network request (seconds). Prevents hanging on large images. Recommended: 180s. |
...
| `image.keep_original_name` | **Renaming Strategy**: If `true`, generates `ShortTitle_OriginalFileName.jpg`; if `false`, only keeps `ShortTitle.jpg` (built-in logic prevents overwriting via auto-increment suffixes). |
...
| Parameter | Description |
| :--- | :--- |
| `system.concurrency` | The number of concurrent threads effective when the `api.endpoints` list is not configured. Default is 1, generally 1-4 is safer. |
